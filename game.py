...
def song_choice():
    ...
    try:
        song_choice_var = int(input(
            "Enter 1 to make the bot always play the same song OR enter 2 to make the bot play the next song each time : "))
    except ValueError:
    ...
    while True:
        ...
        if len(index_nonZero_list) > 0:
            if all(value <= 2 for value in index_nonZero_list):
                ...
                x = (120, 820)
                while True:
                    screen = grab_screen(region=(150, 390, 790, 400))
